...
namespace puzzle02
{
    class Program
    {
        ...
        {
            var spreadsheet = PuzzleInput.Split('\n', StringSplitOptions.RemoveEmptyEntries)
                                         .Select(line => line.Trim())
                                         .Where(line => !String.IsNullOrWhiteSpace(line))
                                         .Select(line => line.Split('\t').Select(str => Convert.ToInt32(str)).ToArray())
                                         .ToArray();

            var total = spreadsheet
                .Select(row => GetDiffBetweenMinAndMax(row))
                .Sum();

            Console.WriteLine(total);
        }

        static int GetDiffBetweenMinAndMax(int[] list)
        {
            var min = list[0];
            var max = list[0];
